...
import { updateObjective, deleteObjective } from '@/server/actions/engagement';
...
export const PATCH = withAccess(
  ...
  async (req: NextRequest, context: { params: Promise<Record<string, string>> }, session: Session) => {
    ...
    try {
      ...
      const data = await updateObjective(objectiveId, body, session.user.id, session.user.name);
...
export const DELETE = withAccess(
  ...
  async (_req: NextRequest, context: { params: Promise<Record<string, string>> }, session: Session) => {
    const { objectiveId } = await context.params;
    try {
      const data = await deleteObjective(objectiveId, session.user.id, session.user.name);
      return Response.json({ data });
    } catch (error) {
      const message = (error as Error).message;
