...
import { getRiskAssessmentFactorById, updateRiskAssessmentFactor, deleteRiskAssessmentFactor } from '@/server/actions/risk-factors';
...
export const GET = withAccess(
  ...
  async (_req: NextRequest, context: { params: Promise<Record<string, string>> }, _session: Session) => {
    ...
    try {
      const data = await getRiskAssessmentFactorById(id);
...
export const PATCH = withAccess(
  ...
  async (req: NextRequest, context: { params: Promise<Record<string, string>> }, session: Session) => {
    ...
    try {
      ...
      const data = await updateRiskAssessmentFactor(id, body, session.user.id, session.user.name);
      return Response.json({ data });
    } catch (error) {
      const message = (error as Error).message;
      if (message.includes('not found')) {
...
export const DELETE = withAccess(
  ...
  async (_req: NextRequest, context: { params: Promise<Record<string, string>> }, session: Session) => {
    const { id } = await context.params;
    try {
      const data = await deleteRiskAssessmentFactor(id, session.user.id, session.user.name);
      return Response.json({ data });
    } catch (error) {
      const message = (error as Error).message;
      if (message.includes('not found')) {
