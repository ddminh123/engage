...
import { listAssessmentSources, createAssessmentSource } from '@/server/actions/assessment-sources';
...
export const GET = withAccess(
  ...
  async (req: NextRequest, _ctx: { params: Promise<Record<string, string>> }, _session: Session) => {
    ...
    const data = await listAssessmentSources(includeInactive);
...
export const POST = withAccess(
  ...
  async (req: NextRequest, _ctx: { params: Promise<Record<string, string>> }, session: Session) => {
    try {
      ...
      const data = await createAssessmentSource(body, session.user.id, session.user.name);
